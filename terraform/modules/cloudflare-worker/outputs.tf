...
output "worker_url" {
  ...
  value = (
    var.workers_dev_subdomain != null && length(trimspace(var.workers_dev_subdomain)) > 0
    ? "https://${cloudflare_worker.this.name}.${var.workers_dev_subdomain}.workers.dev"
    : "https://${cloudflare_worker.this.name}.workers.dev"
  )
}
